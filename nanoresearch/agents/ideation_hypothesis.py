...
Return ONLY valid JSON."""
        ...
        # P0-2: Inject gate feedback from a previous PIVOT, if any. This is
        # set on the agent in IdeationAgent.run() from inputs["_gate_feedback"].
        gate_block = self._format_gate_feedback(
            getattr(self, "_pending_gate_feedback", None)
        )
        if gate_block:
            prompt = gate_block + "\n\n" + prompt
