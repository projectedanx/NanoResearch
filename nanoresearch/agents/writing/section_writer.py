...
from nanoresearch.agents._self_convergence import (
    DEFAULT_MAX_ROUNDS,
    DEFAULT_THRESHOLD,
    ConvergenceEval,
    converge,
...
# Sections that are worth running self-convergence on. Method / Experiments
# are the highest-leverage sections (reviewers focus there) and benefit most
# from gap-driven refinement. Other sections are short and the extra LLM call
# is rarely worth it.
SELF_CONVERGENCE_SECTIONS = frozenset({"Method", "Experiments"})


# JSON schema the section-quality eval LLM should return.
_SECTION_EVAL_SYSTEM = """\
You are a research-paper section quality evaluator. Score the given section
on a 1-10 scale and list specific gaps.

Return JSON:
{
  "coverage_score": <integer 1-10>,
  "missing_directions": ["<specific gap that should be addressed>", ...],
  "reason": "<one-sentence rationale>"
}

Scoring rubric:
- 10: publication-ready; all key points covered with technical depth and clear flow.
- 8-9: solid; minor wording issues but no missing content.
- 6-7: workable but missing a key sub-topic or lacking technical depth.
- 4-5: significant gaps (e.g. missing motivation, undefined notation, no equations).
- 1-3: incoherent, off-topic, or essentially placeholder text.

Return at most 4 missing_directions, each 1 sentence and ACTIONABLE
(e.g. "Add the loss function definition", not "Improve clarity")."""
...
- Use bare ~ for non-breaking space before \\ref (e.g. Figure~\\ref{{fig:arch}}),
  NOT \\~{{}} which renders as a tilde accent character in LaTeX."""
    ...
    # ---- self-convergence wrapper (P0-3) -------------------------------------

    async def _generate_section_with_convergence(
        self, context: str, heading: str, instructions: str,
        prior_sections: list[str] | None = None,
    ) -> str:
        """Generate ``heading`` and self-refine it using the ideation-style
        coverage_score >= 8 OR max_rounds=2 loop.

        Only the high-leverage sections in :data:`SELF_CONVERGENCE_SECTIONS`
        get the extra LLM evaluator call. Everything else falls through to
        the plain :meth:`_generate_section` for cost reasons (mirrors
        ``IdeationAgent``'s pattern of bounding the loop at 2 rounds).
        """
        if heading not in SELF_CONVERGENCE_SECTIONS:
            return await self._generate_section(
                context, heading, instructions, prior_sections,
            )

        # Round-1 generation: identical to the plain path.
        initial_content = await self._generate_section(
            context, heading, instructions, prior_sections,
        )
        if not initial_content or initial_content.startswith("% Section generation failed"):
            return initial_content

        async def _eval(content: str) -> ConvergenceEval:
            return await self._evaluate_section_quality(heading, content)

        async def _refine(content: str, eval_result: ConvergenceEval) -> str:
            return await self._refine_section_with_gaps(
                content, heading, instructions, context,
                eval_result.missing,
            )

        refined, trace = await converge(
            artifact=initial_content,
            eval_fn=_eval,
            refine_fn=_refine,
            threshold=DEFAULT_THRESHOLD,
            max_rounds=DEFAULT_MAX_ROUNDS,
            label=f"writing.{heading}",
        )
        if trace.history:
            self.log(
                f"  {heading}: convergence "
                f"{'reached' if trace.converged else 'capped'} after "
                f"{trace.rounds} round(s), final score "
                f"{trace.final_score}/{DEFAULT_THRESHOLD}"
            )
        return refined

    async def _evaluate_section_quality(
        self, heading: str, content: str,
    ) -> ConvergenceEval:
        """LLM self-evaluation of one section. Fail-open: returns score=10 on
        any internal error so the convergence loop terminates rather than
        spinning."""
        # Truncate so we don't blow the context budget on huge sections.
        snippet = content if len(content) <= 6000 else content[:6000] + "\n...[truncated]"
        prompt = (
            f"Section: {heading}\n\n"
            f"Section content (LaTeX):\n```latex\n{snippet}\n```\n\n"
            f"Evaluate the coverage and quality."
        )
        try:
            payload = await self.generate_json(_SECTION_EVAL_SYSTEM, prompt)
            if not isinstance(payload, dict):
                return ConvergenceEval(score=10)
            raw_score = payload.get("coverage_score", 10)
            if isinstance(raw_score, (int, float)):
                score = max(1, min(10, int(raw_score)))
            else:
                score = 10
            missing = [
                str(m) for m in payload.get("missing_directions", []) if m
            ][:4]
            reason = str(payload.get("reason", ""))
            return ConvergenceEval(score=score, missing=missing, reason=reason)
        except Exception as e:
            logger.warning(
                "Section quality eval failed for '%s' (fail-open): %s",
                heading, e,
            )
            return ConvergenceEval(score=10)

    async def _refine_section_with_gaps(
        self, current_content: str, heading: str, instructions: str,
        context: str, missing: list[str],
    ) -> str:
        """Re-generate ``heading`` addressing the listed gaps. The original
        content is shown to the LLM as a starting point so it preserves the
        existing structure while filling in the missing pieces."""
        if not missing:
            return current_content

        gap_lines = "\n".join(f"  - {g}" for g in missing)
        section_system = get_writing_system_prompt(heading)
        prompt = f"""Revise the "{heading}" section to address the following gaps.
...
Specific gaps to fix (mandatory):
{gap_lines}
...
CRITICAL RULES:
1. Address EVERY gap above -- each one must be reflected in the revision.
2. Preserve all parts of the current section that are NOT mentioned in the gaps.
3. Do NOT shorten the section -- only add / improve the listed weak points.
4. Do NOT introduce new \\begin{{figure}}...\\end{{figure}} blocks.
5. Do NOT wrap output in Markdown code fences.
6. Use ONLY citation keys that appear in the current section's references.

Output ONLY the revised LaTeX paragraphs (no \\section command)."""

        try:
            refined = ((await self.generate(section_system, prompt)) or "").strip()
            refined = re.sub(r'\\end\{document\}\s*', '', refined).strip()
            refined = self._strip_leading_filler(refined)
            refined = _strip_llm_thinking(refined)
            refined = self._fix_component_count_mismatch(refined)
            # Defensive: if the refine returned something obviously broken
            # (much shorter than original), keep the original.
            if len(refined) < len(current_content) * 0.5:
                logger.warning(
                    "Refined %s shrank by >50%% (%d -> %d chars), keeping original",
                    heading, len(current_content), len(refined),
                )
                return current_content
            return refined
        except Exception as e:
            logger.warning("Section refine failed for '%s': %s", heading, e)
            return current_content
