    ...
    async def _generate_ai_figure(
        ...
        def _short_method_prompt() -> str:
            ...
            prompt = (
                ...
                "Use five visually distinct modules from left to right: data table input, stratified split, "
                "training-fold preprocessing, NSGA-II feature-mask search, Pareto selection and held-out evaluation. "
                "Make the central search module larger with a circular evolutionary loop, small binary-mask tiles, "
                "two objective callouts labeled CV balanced accuracy and selected features, and a highlighted Pareto front. "
                "Show leakage safety by a dashed barrier that keeps the test set outside the search loop. "
