...
class _CodeFigureMixin:
    ...
    async def _generate_code_figure(
        ...
        for attempt in range(MAX_CODE_CHART_RETRIES):
            ...
            # NOTE: the `from matplotlib import .*` arm intentionally has NO
            # `(?:\.\w+)?` submodule wildcard.  An earlier version had it and
            # silently stripped `from matplotlib.patches import Patch` (and
            # similar lines for matplotlib.colors / matplotlib.lines /
            # matplotlib.ticker), causing a deterministic NameError loop —
            # the LLM kept re-adding the import and we kept deleting it.
            # See 汇报_2026-04-07 §6.11.4 for the full post-mortem.
            code = re.sub(
