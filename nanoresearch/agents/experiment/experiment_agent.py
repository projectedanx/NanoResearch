...
class _ExperimentAgentMixin:
    ...
    async def run(self, **inputs: Any) -> dict[str, Any]:
        ...
        # Self-evolution: pull adaptive context for the experiment task type
        # before doing any LLM work.  See §6.13 / §1.5.
        ...
        # Self-evolution: prepend adaptive context (skills + memory) to repo
        # context so the LLM grounding packet has both static and learned
        # knowledge.
        if adaptive_context:
            repo_context = (
        ...
        for round_num in range(start_round, max_rounds + 1):
            ...
            if preflight.overall_status == "failed":
                ...
                # Self-evolution: capture the preflight failure as a learnable
                # trace so future rounds can pull this rule via skills.
                failure_summary = "; ".join(preflight.blocking_failures)
                fix_summary = "; ".join(preflight.suggested_fixes[:5])
                self.learn_from_trace(
                    "experiment",
                    "preflight_failure",
                    f"Round {round_num} preflight failures: {failure_summary} "
                    f"| suggested fixes: {fix_summary}",
        ...
        # Self-evolution: capture experiment status, decision history, retry-
        # success traces, strategy summary, and any failed-direction record.
        # See §6.13 / §1.5.
        metrics_summary = str(best_round_data["metrics"])
        self.remember_context(
            MemoryType.PROJECT_CONTEXT,
            f"Experiment status for {title}: final_status={iteration_state.final_status}, "
            f"best_round={iteration_state.best_round}, metrics={metrics_summary}",
            importance=0.82,
        ...
        self.remember_context(
            ...
            f"Experiment loop for {title} ended with execution={execution_status} "
            f"and quick_eval={quick_eval_status} after {len(iteration_state.rounds)} rounds.",
            importance=0.75,
        ...
        if len(iteration_state.rounds) > 1 and iteration_state.best_round > 1:
            self.learn_from_trace(
                "experiment",
                "retry_success",
                f"Best result for {title} emerged at round {iteration_state.best_round} "
                f"after iterative retries. Preserve successful fixes from earlier failed rounds.",
