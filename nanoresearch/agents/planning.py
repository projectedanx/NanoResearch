...
class PlanningAgent(BaseResearchAgent):
    ...
    async def run(self, **inputs: Any) -> dict[str, Any]:
        ...
        # Self-evolution: pull adaptive context for planning task type before
        # any LLM work.  See §6.13 / §1.5.
...
Return ONLY valid JSON."""
        ...
        # P0-2: Inject gate feedback from a previous PIVOT, if any. The
        # orchestrator surfaces this via inputs["_gate_feedback"] when a
        # downstream gate (PLANNING or QUALITY) sent us back to be rerun.
        gate_block = self._format_gate_feedback(inputs.get("_gate_feedback"))
        if gate_block:
            prompt = gate_block + "\n\n" + prompt
            self.log(
                "Gate feedback injected into planning prompt "
                f"({inputs['_gate_feedback'].get('gate_name', 'GATE')})"
            )
        ...
        # Self-evolution: capture project context (method/datasets/metrics)
        # + planning constraints + promising direction + a planning trace.
        # See §6.13 / §1.5.
        primary_metrics = (
            [m.name for m in blueprint.metrics if m.primary]
            or [m.name for m in blueprint.metrics[:3]]
        )
        primary_metrics_text = ", ".join(primary_metrics)
        dataset_names = ", ".join(ds.name for ds in blueprint.datasets[:3])
        # NOTE: blueprint.proposed_method is declared as `dict` in
        # schemas/experiment.py:119, NOT a pydantic submodel — must use
        # dict access (.get), not attribute access (.name).  Upstream
        # self-evolution had this bug; do not propagate it.
        method_name = blueprint.proposed_method.get("name", "Proposed Method")
        self.remember_context(
            MemoryType.PROJECT_CONTEXT,
            f"Planning blueprint for {topic}: method={method_name}, "
            f"datasets={dataset_names}, primary metrics={primary_metrics_text}",
            importance=0.78,
        ...
        self.remember_context(
            ...
            f"Planning constraints for {topic}: include {len(blueprint.ablation_groups)} "
            f"ablation groups and compute estimate {blueprint.compute_requirements.num_gpus}x "
            f"{blueprint.compute_requirements.gpu_type} for "
            f"{blueprint.compute_requirements.estimated_hours}h.",
            importance=0.72,
        ...
        planning_trace = (
            ...
            f"compute={blueprint.compute_requirements.num_gpus}x "
            f"{blueprint.compute_requirements.gpu_type} "
