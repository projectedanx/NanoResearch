...
class IdeationAgent(_IdeationSearchMixin, _IdeationHypothesisMixin, BaseResearchAgent):
    ...
    async def run(self, **inputs: Any) -> dict[str, Any]:
        ...
        # P0-2: capture orchestrator gate feedback so the LLM hypothesis-
        # generation step (in ideation_hypothesis._analyze_and_hypothesize)
        # can include it in its prompt and avoid the same mistakes on rerun.
        self._pending_gate_feedback = inputs.get("_gate_feedback")
        if self._pending_gate_feedback:
            self.log(
                f"Gate feedback present from previous attempt "
                f"({self._pending_gate_feedback.get('gate_name', 'GATE')}); "
                f"will inject into hypothesis prompt"
            )
        logger.info("[%s] Starting ideation for topic: %s", self.stage.value, topic)

        # Self-evolution: pull adaptive context (skills + memory + past failures)
        # for this topic before doing any LLM work.  See §6.13 / §1.5 of the
        # 汇报_2026-04-07 doc for the rationale.
        ...
        # Self-evolution: capture project context + key gaps + promising direction
        # so future ideation runs (and downstream stages) can recall what we
        # tried and learned for this topic.  See §6.13 / §1.5.
        gap_descriptions = [gap.description for gap in output.gaps[:3]]
        gap_summary = "; ".join(gap_descriptions)
        self.remember_context(
            MemoryType.PROJECT_CONTEXT,
            f"Ideation for {topic} selected {output.selected_hypothesis} "
            f"with rationale: {output.rationale}",
            importance=0.74,
