...
class Workspace(_WorkspaceExportMixin):
    ...
    def mark_stage_pending(self, stage: PipelineStage) -> None:
        """Reset a stage back to ``pending`` (used by gate PIVOTs).

        Clears completion / failure metadata so the orchestrator will actually
        re-execute the stage on the next iteration of its run loop instead of
        skipping it via the "already completed" branch.
        """
        m = self.manifest
        rec = m.stages.get(stage.value)
        if rec is None:
        ...
        rec.status = "pending"
        rec.error_message = ""
        # Leave started_at / completed_at alone for audit trail.
        self._write_manifest(m)
