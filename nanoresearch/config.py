...
class ResearchConfig(BaseModel):
    ...
    figure_prompt: StageModelConfig = Field(
        default_factory=lambda: StageModelConfig(
            # NOTE: was "claude-sonnet-4-6-20250514" — that exact name does NOT
            # exist on the boyuerichdata / siliconcloud proxies. The real name
            # is "claude-sonnet-4-6" without the date suffix. Verified by
            # querying GET /v1/models on 2026-04-08.
    ...
    figure_code: StageModelConfig = Field(
        default_factory=lambda: StageModelConfig(
            # Same fix as figure_prompt above — drop the date suffix.
    ...
    figure_gen: StageModelConfig = Field(
        default_factory=lambda: StageModelConfig(
            # Using the high-quality "pro" variant for paper-grade architecture
            # diagrams. The boyuerichdata proxy occasionally truncates large
            # image responses (IncompleteRead), but the upstream retry logic
            # in pipeline/_multi_model_helpers.py:_generate_image_gemini was
            # patched 2026-04-08 to retry httpx.RemoteProtocolError up to 6
            # times with exponential backoff + jitter. This keeps the long,
            # detailed prompt (~3800 chars) intact and lets the retry loop
            # absorb proxy flakiness.
    ...
    revision: StageModelConfig = Field(
        default_factory=lambda: StageModelConfig(
            # NOTE: was "gemini-3-pro-preview-thinking" — that name does not
            # exist on the boyuerichdata proxy. The real name has the 3.1
            # version suffix: "gemini-3.1-pro-preview-thinking". Verified by
            # querying GET /v1/models on 2026-04-08.
    ...
    # Adaptive memory and skill evolution settings (legacy `evolution/` module)
    # Kept for backward compatibility with main branch; DEV's new memory.py + skill_registry.py
    # are still active regardless of these flags.
